...
function TextHighlighter({ output }) {
  ...
  const highlightText = () => {
    if (selection) {
      ...
      const parentElement =
        selection.commonAncestorContainer.nodeType === Node.TEXT_NODE
      ...
      const starting_index = parentText.indexOf(
        selectedText,
  ...
  const undoLastAnnotation = () => {
    if (undo.length > 0) {
      ...
      while (lastAnnotation.element.firstChild) {
        parent.insertBefore(
          lastAnnotation.element.firstChild,
          lastAnnotation.element
        );
      }
  ...
  return (
    <div>
      <p>
        Select the text to label it as error and finally <strong>submit</strong>{" "}
        your annotation.
      </p>
      <p
